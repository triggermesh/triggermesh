...
- `SYSTEM_NAMESPACE`
   ...
   (ConfigMap objects). This can potentially be set to any namespace, including `default`, since the controller falls
   back to a default configuration if the expected ConfigMaps are missing.

- `METRICS_DOMAIN`

   The domain to use for surfacing [Prometheus metrics][prom]. During development, any placeholder value can be used.
   The variable can be omitted altogether under the special condition that the namespace defined by `SYSTEM_NAMESPACE`
   does not actually contain a ConfigMap for observability (`config-observability` by default).
...
<!-- TODO warn about conflicting controllers -->
