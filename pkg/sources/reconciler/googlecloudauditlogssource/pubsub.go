...
// subscription for receiving change notifications from a Cloud Audit Logs resource.
...
// for receiving change notifications from a Cloud Audit Logs resource are deleted.
...
// change notifications from the Cloud Audit Logs resource.
...
// messages from the Cloud Audit Logs resource is deleted.
// Required permissions:
// - pubsub.topics.get
...
func ensurePubSubSubscription(ctx context.Context, cli *pubsub.Client, topicResName *v1alpha1.GCloudResourceName) error {
	...
	if !status.GetCondition(v1alpha1.GoogleCloudAuditLogsConditionSubscribed).IsTrue() {
...
func pubsubResourceID(src commonv1alpha1.Reconcilable) string {
	return src.GetNamespace() + "." + src.GetName() + "~" + sources.GoogleCloudAuditLogsSourceResource.String()
}

// assertPubSubTopicOwnership returns whether a Pub/Sub topic is owned by the
// given source.
func assertPubSubTopicOwnership(src *v1alpha1.GoogleCloudAuditLogsSource, topicCfg pubsub.TopicConfig) bool {
...
func assertPubSubSubscriptionOwnership(src *v1alpha1.GoogleCloudAuditLogsSource, subsCfg pubsub.SubscriptionConfig) bool {
	for k, v := range pubsubResourceLabels(src) {
...
func pubsubResourceLabels(src *v1alpha1.GoogleCloudAuditLogsSource) map[string]string {
	return map[string]string{
		pubsubLabelOwnerResource:  strings.ReplaceAll(sources.GoogleCloudAuditLogsSourceResource.String(), ".", "-"),
