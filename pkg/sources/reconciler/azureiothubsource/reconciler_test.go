...
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		...
		r.base = NewTestDeploymentReconciler[*v1alpha1.AzureIOTHubSource](ctx, ls,
			ls.GetAzureIOTHubSourceLister().AzureIOTHubSources,
		)

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetAzureIOTHubSourceLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newEventSource returns a populated source object.
func newEventSource() *v1alpha1.AzureIOTHubSource {
	src := &v1alpha1.AzureIOTHubSource{
		Spec: v1alpha1.AzureIOTHubSourceSpec{
