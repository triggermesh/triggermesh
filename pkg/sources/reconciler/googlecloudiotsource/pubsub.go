...
// subscription for receiving change notifications from a Cloud Source IoT resource.
...
// for receiving change notifications from a Cloud Source IoT resource are deleted.
...
// change notifications from the Cloud Source IoT resource.
...
// messages from the Cloud Source IoT resource is deleted.
// Required permissions:
// - pubsub.topics.get
...
func ensurePubSubSubscription(ctx context.Context, cli *pubsub.Client, topicResName *v1alpha1.GCloudResourceName) error {
	...
	if !status.GetCondition(v1alpha1.GoogleCloudIoTConditionSubscribed).IsTrue() {
...
// given source.
func assertPubSubTopicOwnership(src *v1alpha1.GoogleCloudIoTSource, topicCfg pubsub.TopicConfig) bool {
...
func assertPubSubSubscriptionOwnership(src *v1alpha1.GoogleCloudIoTSource, subsCfg pubsub.SubscriptionConfig) bool {
	for k, v := range pubsubResourceLabels(src) {
...
func pubsubResourceLabels(src *v1alpha1.GoogleCloudIoTSource) map[string]string {
	return map[string]string{
		pubsubLabelOwnerResource:  strings.ReplaceAll(sources.GoogleCloudIoTSourceResource.String(), ".", "-"),
