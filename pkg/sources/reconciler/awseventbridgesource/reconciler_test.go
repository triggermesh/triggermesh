...
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		r := &Reconciler{
			adapterCfg: cfg,
			ebCg:       staticClientGetter(nil, nil),
		}

		r.base = NewTestDeploymentReconciler[*v1alpha1.AWSEventBridgeSource](ctx, ls,
			ls.GetAWSEventBridgeSourceLister().AWSEventBridgeSources,
		)

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetAWSEventBridgeSourceLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newEventSource returns a test source object with a minimal set of pre-filled attributes.
func newEventSource() *v1alpha1.AWSEventBridgeSource {
	src := &v1alpha1.AWSEventBridgeSource{
		Spec: v1alpha1.AWSEventBridgeSourceSpec{
			ARN: NewARN(eventbridge.EndpointsID, "event-bus/triggermeshtest"),
		},
	}

	// assume finalizer is already set to prevent the generated reconciler
	// from generating an extra Patch action
	src.Finalizers = []string{sources.AWSEventBridgeSourceResource.String()}
...
// ClientGetter.
func staticClientGetter(ebCli eventbridgeclient.Client, sqsCli eventbridgeclient.SQSClient) eventbridgeclient.ClientGetterFunc {
	return func(*v1alpha1.AWSEventBridgeSource) (eventbridgeclient.Client, eventbridgeclient.SQSClient, error) {
		return ebCli, sqsCli, nil
	}
}
