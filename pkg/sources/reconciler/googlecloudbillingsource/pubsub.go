...
// notifications from the Cloud Billing budget.
...
// messages from the Cloud Billing budget is deleted.
// Required permissions:
// - pubsub.topics.get
...
func ensurePubSubSubscription(ctx context.Context, cli *pubsub.Client, topicResName *v1alpha1.GCloudResourceName) error {
	...
	if !status.GetCondition(v1alpha1.GoogleCloudBillingConditionSubscribed).IsTrue() {
...
// given source.
func assertPubSubTopicOwnership(src *v1alpha1.GoogleCloudBillingSource, topicCfg pubsub.TopicConfig) bool {
...
func assertPubSubSubscriptionOwnership(src *v1alpha1.GoogleCloudBillingSource, subsCfg pubsub.SubscriptionConfig) bool {
	for k, v := range pubsubResourceLabels(src) {
...
func pubsubResourceLabels(src *v1alpha1.GoogleCloudBillingSource) map[string]string {
	return map[string]string{
		pubsubLabelOwnerResource:  strings.ReplaceAll(sources.GoogleCloudBillingSourceResource.String(), ".", "-"),
