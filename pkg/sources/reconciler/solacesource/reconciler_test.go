...
import (
	...
	reconcilerv1alpha1 "github.com/triggermesh/triggermesh/pkg/client/generated/injection/reconciler/sources/v1alpha1/solacesource"
...
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		...
		r.base = NewTestDeploymentReconciler[*v1alpha1.SolaceSource](ctx, ls,
			ls.GetSolaceSourceLister().SolaceSources,
		)

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetSolaceSourceLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newEventSource returns a test source object with a minimal set of pre-filled attributes.
func newEventSource() *v1alpha1.SolaceSource {
	username := "admin"
	saslEnable := true
	src := &v1alpha1.SolaceSource{
		Spec: v1alpha1.SolaceSourceSpec{
			URL:       "localhost:9092",
			QueueName: "test-topic",
			Auth: &v1alpha1.SolaceSourceAuth{
