...
const (
	...
	tQuery        = ".foo | .."
	tJSON         = `{"foo":"richard@triggermesh.com"}`
...
func TestReplier(t *testing.T) {
	testCases := map[string]struct {
		...
		"transform error": {
			inEvent: newCloudEvent(t, tFalseJSON, cloudevents.ApplicationXML),

			expectEvent: newCloudEvent(t, tExpectedFailure, cloudevents.ApplicationXML),
		},
	}
