...
// ComponentName returns the component name for the given component object.
...
func ServiceAccountName(src v1alpha1.EventFlowComponent) string {
	if v1alpha1.WantsOwnServiceAccount(src) {
		srcName := src.GetName()

		// Edge case: we need to make sure some characters are inserted
		// between the component name and the component name to avoid
		// clashing with the shared "{kind}-adapter" ServiceAccount in
		// case the component is named "adapter". We picked 'i' for
...
func NewAdapterDeployment(src v1alpha1.EventFlowComponent, sinkURI *apis.URL, opts ...resource.ObjectOption) *appsv1.Deployment {
...
// Deployment.
func NewMTAdapterDeployment(src v1alpha1.EventFlowComponent, opts ...resource.ObjectOption) *appsv1.Deployment {
	srcNs := src.GetNamespace()
...
func commonAdapterDeploymentOptions(src v1alpha1.EventFlowComponent) []resource.ObjectOption {
...
func NewAdapterKnService(src v1alpha1.EventFlowComponent, sinkURI *apis.URL, opts ...resource.ObjectOption) *servingv1.Service {
	srcNs := src.GetNamespace()
	srcName := src.GetName()
...
// pre-populates attributes common to all multi-tenant adapters backed by a
// Knative Service.
func NewMTAdapterKnService(src v1alpha1.EventFlowComponent, opts ...resource.ObjectOption) *servingv1.Service {
	srcNs := src.GetNamespace()
...
func commonAdapterKnServiceOptions(src v1alpha1.EventFlowComponent) []resource.ObjectOption {
	app := ComponentName(src)
...
func newServiceAccount(src v1alpha1.EventFlowComponent, owners []kmeta.OwnerRefable) *corev1.ServiceAccount {
...
func newRoleBinding(src v1alpha1.EventFlowComponent, owner *corev1.ServiceAccount) *rbacv1.RoleBinding {
