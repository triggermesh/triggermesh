...
import (
	...
	"github.com/triggermesh/triggermesh/pkg/flow/reconciler/common/event"
	"github.com/triggermesh/triggermesh/pkg/flow/reconciler/common/resource"
	"github.com/triggermesh/triggermesh/pkg/flow/reconciler/common/semantic"
	"github.com/triggermesh/triggermesh/pkg/flow/routing"
)
...
type RBACOwnersLister interface {
	RBACOwners(src v1alpha1.EventFlowComponent) ([]kmeta.OwnerRefable, error)
}
...
type AdapterDeploymentBuilder interface {
	RBACOwnersLister
	BuildAdapter(src v1alpha1.EventFlowComponent) *appsv1.Deployment
}
...
// ReconcileAdapter reconciles a component's adapter.
func (r *GenericDeploymentReconciler) ReconcileAdapter(ctx context.Context, ab AdapterDeploymentBuilder) reconciler.Event {
	src := v1alpha1.EventFlowComponentFromContext(ctx)

	desiredAdapter := ab.BuildAdapter(src)
...
// component, or creates it if it is missing.
...
// ReconcileComponent reconciles a receive adapter for a component.
func (r *GenericServiceReconciler) ReconcileComponent(ctx context.Context, ab AdapterServiceBuilder) reconciler.Event {
	src := v1alpha1.EventFlowComponentFromContext(ctx)
...
// findAdapter returns the adapter object for a given component if it exists.
func findAdapter(genericReconciler interface{},
	src v1alpha1.EventFlowComponent, owner *metav1.OwnerReference) (metav1.Object, error) {
...
func (r *GenericRBACReconciler) reconcileRBAC(ctx context.Context,
	...
	// given component. It gets garbage collected by Kubernetes as soon
	// as its last owner (component) is deleted, so we don't need to clean
	// things up explicitly once the last component gets deleted.
	if len(owners) == 0 {
		return nil, nil
	}
...
// for a given component, or creates it if it is missing.
...
func (r *GenericRBACReconciler) syncAdapterServiceAccount(ctx context.Context,
	...
	controller.GetEventRecorder(ctx).Eventf(newNamespace(sa.Namespace), corev1.EventTypeNormal,
		...
		sa.Name, v1alpha1.EventFlowComponentFromContext(ctx).GetGroupVersionKind().Kind)
...
func serviceAccountMutations(src v1alpha1.EventFlowComponent) []resource.ServiceAccountOption {
