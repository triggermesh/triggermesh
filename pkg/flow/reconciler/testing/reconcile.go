...
import (
	...
	eventtesting "github.com/triggermesh/triggermesh/pkg/flow/testing/event"
)
	...
	testCases := rt.TableTest{
...

		{
			...
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: newEventTarget(withSink, notDeployed(a)),
			}},
...

		{
			...
			Objects: []runtime.Object{
				newAddressable(),
				newEventTarget(withSink, notDeployed(a)),
				newServiceAccount(),
				newRoleBinding(),
				newAdapter(ready),
			},
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: newEventTarget(withSink, deployed(a)),
			}},
		},
		{
			...
			Objects: []runtime.Object{
				newAddressable(),
				newEventTarget(withSink, deployed(a)),
				newServiceAccount(),
				newRoleBinding(),
				newAdapter(notReady),
			},
		...
		},
		{
			...
			Objects: []runtime.Object{
				newAddressable(),
				// instance had the SinkProvided status condition set,
				// then spec.sink got deleted by the user
				newEventTarget(withSink, deployed(a), withoutSinkSpec),
				newServiceAccount(),
				newRoleBinding(),
				newAdapter(ready),
			},
			WantStatusUpdates: func() []clientgotesting.UpdateActionImpl {
				...
				if _, isEventSender := rcl.(v1alpha1.EventSender); isEventSender {
					return []clientgotesting.UpdateActionImpl{{
						// the SinkProvided status condition gets cleared
						Object: newEventTarget(withoutSinkSpec, deployed(a)),
					}}
				}
				return nil
			}(),
...

		{
			...
			WantStatusUpdates: func() []clientgotesting.UpdateActionImpl {
				...
				if _, isEventSender := rcl.(v1alpha1.EventSender); isEventSender {
					return []clientgotesting.UpdateActionImpl{{
						Object: newEventTarget(withoutSink, deployed(a)),
					}}
				}
				return nil
			}(),
		...
		},
		{
			...
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: newEventTarget(unknownDeployedWithError(a), withSink),
			}},
			WantEvents: []string{
