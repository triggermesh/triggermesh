...
// GenericAdapterReconciler contains interfaces shared across Deployment reconcilers.
type GenericAdapterReconciler[T kmeta.OwnerRefable, A metav1.Object] struct {
	// URI resolver for sinks
	SinkResolver *resolver.URIResolver
	// API clients
	Client    kClientGetter[A]
	PodClient func(namespace string) coreclientv1.PodInterface
	// objects listers
	Lister kListerGetter[A]

	*GenericRBACReconciler[T]
}

// kClient is a partial generic version of a typed <Kind>NamespaceLister
// interface (usually generated by lister-gen).
type kClient[A metav1.Object] interface {
	Create(context.Context, A, metav1.CreateOptions) (A, error)
	Update(context.Context, A, metav1.UpdateOptions) (A, error)
}

// kClientGetter obtains a namespaced kClient.
type kClientGetter[A metav1.Object] func(namespace string) kClient[A]

// kLister is a partial generic version of a typed <Kind>NamespaceLister
// interface (usually generated by lister-gen).
type kLister[A metav1.Object] interface {
	List(labels.Selector) ([]A, error)
}

// kListerGetter obtains a namespaced kLister.
type kListerGetter[A metav1.Object] func(namespace string) kLister[A]

// GenericRBACReconciler reconciles RBAC objects for components adapters.
type GenericRBACReconciler[T kmeta.OwnerRefable] struct {
	// API clients
...
type ListerGetter[T kmeta.OwnerRefable] func(namespace string) Lister[T]

// NewGenericDeploymentReconciler creates a new GenericAdapterReconciler for an
// adapter backed by a Deployment, and attaches a default event handler to its
// Deployment informer.
func NewGenericDeploymentReconciler[T kmeta.OwnerRefable](ctx context.Context, gvk schema.GroupVersionKind,
	tracker tracker.Interface,
	adapterHandlerFn func(obj interface{}),
	ownersLister ListerGetter[T],
) GenericAdapterReconciler[T, *appsv1.Deployment] {

	informer := deploymentinformerv1.Get(ctx)

	r := GenericAdapterReconciler[T, *appsv1.Deployment]{
		SinkResolver: resolver.NewURIResolverFromTracker(ctx, tracker),
		Client: func(namespace string) kClient[*appsv1.Deployment] {
			return k8sclient.Get(ctx).AppsV1().Deployments(namespace)
		},
		PodClient: k8sclient.Get(ctx).CoreV1().Pods,
		Lister: func(namespace string) kLister[*appsv1.Deployment] {
			return informer.Lister().Deployments(namespace)
		},
		GenericRBACReconciler: NewGenericRBACReconciler[T](ctx, ownersLister),
	}
...
// NewGenericServiceReconciler creates a new GenericAdapterReconciler for an
// adapter backed by a Knative Service, and attaches a default event handler to
// its Service informer.
func NewGenericServiceReconciler[T kmeta.OwnerRefable](ctx context.Context, gvk schema.GroupVersionKind,
	tracker tracker.Interface,
	adapterHandlerFn func(obj interface{}),
	ownersLister ListerGetter[T],
) GenericAdapterReconciler[T, *servingv1.Service] {
	...
	return newGenericServiceReconciler[T](ctx, tracker, ownersLister)
}

// NewMTGenericServiceReconciler creates a new GenericAdapterReconciler for a
// multi-tenant adapter backed by a Knative Service, and attaches a default
// event handler to its Service informer.
func NewMTGenericServiceReconciler[T kmeta.OwnerRefable](ctx context.Context, typ kmeta.OwnerRefable,
	tracker tracker.Interface,
	adapterHandlerFn func(obj interface{}),
	ownersLister ListerGetter[T],
) GenericAdapterReconciler[T, *servingv1.Service] {
...
func NewGenericRBACReconciler[T kmeta.OwnerRefable](ctx context.Context,
	ownersLister ListerGetter[T],
) *GenericRBACReconciler[T] {

	return &GenericRBACReconciler[T]{
...
func newGenericServiceReconciler[T kmeta.OwnerRefable](ctx context.Context,
	tracker tracker.Interface,
	ownersLister ListerGetter[T],
) GenericAdapterReconciler[T, *servingv1.Service] {

	return GenericAdapterReconciler[T, *servingv1.Service]{
		SinkResolver: resolver.NewURIResolverFromTracker(ctx, tracker),
		Client: func(namespace string) kClient[*servingv1.Service] {
			return servingclient.Get(ctx).ServingV1().Services(namespace)
		},
		Lister: func(namespace string) kLister[*servingv1.Service] {
			return serviceinformerv1.Get(ctx).Lister().Services(namespace)
		},
		GenericRBACReconciler: NewGenericRBACReconciler[T](ctx, ownersLister),
	}
