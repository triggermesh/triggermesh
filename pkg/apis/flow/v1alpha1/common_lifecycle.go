...
import (
	...
	"github.com/triggermesh/triggermesh/pkg/flow/status"
)
...
// eventFlowConditionSet is a generic set of status conditions used by
// default in all flow components.
var eventFlowConditionSet = NewEventFlowConditionSet()

// NewEventFlowConditionSet returns a set of status conditions for a flow
// component. Default conditions can be augmented by passing condition types as
// function arguments.
func NewEventFlowConditionSet(cts ...apis.ConditionType) apis.ConditionSet {
	return apis.NewLivingConditionSet(
		append(eventFlowConditionTypes, cts...)...,
	)
}

// eventFlowConditionTypes is a list of condition types common to all event
// flow components.
var eventFlowConditionTypes = []apis.ConditionType{
	ConditionDeployed,
}

// EventFlowStatusManager manages the status of flow components.
//
// +k8s:deepcopy-gen=false
type EventFlowStatusManager struct {
	apis.ConditionSet
	*EventFlowStatus
}
...
func (m *EventFlowStatusManager) MarkRBACNotBound() {
...
// Deployment.
func (m *EventFlowStatusManager) PropagateDeploymentAvailability(ctx context.Context,
...
func (m *EventFlowStatusManager) PropagateServiceAvailability(ksvc *servingv1.Service) {
	if ksvc == nil {
		m.ConditionSet.Manage(m).MarkUnknown(ConditionDeployed, ReasonUnavailable,
...
// SetRoute appends the given URL path to the current component's URL.
func (m *EventFlowStatusManager) SetRoute(urlPath string) {
