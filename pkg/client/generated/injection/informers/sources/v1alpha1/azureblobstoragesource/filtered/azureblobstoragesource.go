...
func withInformer(ctx context.Context) (context.Context, []controller.Informer) {
	...
	for _, selector := range labelSelectors {
		...
		inf := f.Sources().V1alpha1().AzureBlobStorageSources()
...
func Get(ctx context.Context, selector string) v1alpha1.AzureBlobStorageSourceInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch github.com/triggermesh/triggermesh/pkg/client/generated/informers/externalversions/sources/v1alpha1.AzureBlobStorageSourceInformer with selector %s from context.", selector)
	}
	return untyped.(v1alpha1.AzureBlobStorageSourceInformer)
}
...
var _ sourcesv1alpha1.AzureBlobStorageSourceLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apissourcesv1alpha1.AzureBlobStorageSource{}, 0, nil)
}

func (w *wrapper) Lister() sourcesv1alpha1.AzureBlobStorageSourceLister {
	return w
}
