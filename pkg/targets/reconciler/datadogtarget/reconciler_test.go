...
// reconcilerCtor returns a Ctor for a DatadogTarget Reconciler.
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		...
		r.base = NewTestServiceReconciler[*v1alpha1.DatadogTarget](ctx, ls,
			ls.GetDatadogTargetLister().DatadogTargets,
		)

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetDatadogTargetLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newTarget returns a populated target object.
func newTarget() *v1alpha1.DatadogTarget {
	trg := &v1alpha1.DatadogTarget{
		Spec: v1alpha1.DatadogTargetSpec{},
	}
