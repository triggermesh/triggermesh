...
var (
	...
	tToken = &corev1.SecretKeySelector{
		LocalObjectReference: corev1.LocalObjectReference{Name: "jira-secret"},
		Key:                  "jira-key",
	}
)
...
// reconcilerCtor returns a Ctor for a JiraTarget Reconciler.
var reconcilerCtor Ctor = func(t *testing.T, ctx context.Context, ls *Listers) controller.Reconciler {
	...
	return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
		fakeinjectionclient.Get(ctx), ls.GetJiraTargetLister(),
		controller.GetEventRecorder(ctx), r)
}

/* Event targets */

// newEventTarget returns a test JiraTarget object with pre-filled attributes.
func newEventTarget() *v1alpha1.JiraTarget {
	o := &v1alpha1.JiraTarget{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: tNs,
			Name:      tName,
			UID:       tUID,
			Labels: labels.Set{
				reconciler2.LabelBridgeUsedByPrefix + tBridge: reconciler2.LabelValueBridgeDominant,
			},
		},
		Spec: v1alpha1.JiraTargetSpec{
			Auth: v1alpha1.JiraAuth{
				User:  tUser,
				Token: v1alpha1.SecretValueFromSource{SecretKeyRef: tToken},
			},
			URL: tURL,
		},
	}
...
// Deployed: Unknown
func newEventTargetUnknownDeployed(adapterExists bool) *v1alpha1.JiraTarget {
	o := newEventTarget()
...
// Deployed: True
func newEventTargetDeployed() *v1alpha1.JiraTarget {
	o := newEventTarget()
...
// Deployed: False
func newEventTargetNotDeployed() *v1alpha1.JiraTarget {
	o := newEventTarget()
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{
				*kmeta.NewControllerRef(NewOwnerRefable(
					tName,
					(&v1alpha1.JiraTarget{}).GetGroupVersionKind(),
					tUID,
				)),
			},
		},
