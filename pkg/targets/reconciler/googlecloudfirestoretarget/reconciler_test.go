...
// reconcilerCtor returns a Ctor for a GoogleCloudFirestoreTarget Reconciler.
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		r := &Reconciler{
			base:       NewTestServiceReconciler(ctx, ls),
			adapterCfg: cfg,
			trgLister:  ls.GetGoogleCloudFirestoreTargetLister().GoogleCloudFirestoreTargets,
		}

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetGoogleCloudFirestoreTargetLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newTarget returns a populated target object.
func newTarget() *v1alpha1.GoogleCloudFirestoreTarget {
	trg := &v1alpha1.GoogleCloudFirestoreTarget{
		Spec: v1alpha1.GoogleCloudFirestoreTargetSpec{
			DefaultCollection: "triggermeshtest",
		},
	}
