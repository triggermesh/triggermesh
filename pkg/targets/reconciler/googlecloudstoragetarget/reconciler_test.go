...
// reconcilerCtor returns a Ctor for a GoogleCloudStorageTarget Reconciler.
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		...
		r.base = NewTestServiceReconciler(
			ctx,
			ls,
			func(namespace string) common.Lister[*v1alpha1.GoogleCloudStorageTarget] {
				return ls.GetGoogleCloudStorageTargetLister().GoogleCloudStorageTargets(namespace)
			},
		)

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetGoogleCloudStorageTargetLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newTarget returns a populated target object.
func newTarget() *v1alpha1.GoogleCloudStorageTarget {
	trg := &v1alpha1.GoogleCloudStorageTarget{
		Spec: v1alpha1.GoogleCloudStorageTargetSpec{
			BucketName: "triggermeshtest",
		},
	}
