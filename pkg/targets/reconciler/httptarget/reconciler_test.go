...
const (
	...
	tResponseSource = "response.source"
	tSkipVerify     = "false"
)
...
var reconcilerCtor Ctor = func(t *testing.T, ctx context.Context, ls *Listers) controller.Reconciler {
	...
	return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
		fakeinjectionclient.Get(ctx), ls.GetHTTPTargetLister(),
		controller.GetEventRecorder(ctx), r)
}

/* Event targets */

// newEventTarget returns a test HTTPTarget object with pre-filled attributes.
func newEventTarget() *v1alpha1.HTTPTarget {
	o := &v1alpha1.HTTPTarget{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: tNs,
			Name:      tName,
			UID:       tUID,
		},
		Spec: v1alpha1.HTTPTargetSpec{
			Endpoint: tEndpointURL,
			Method:   tMethod,
			Response: v1alpha1.HTTPEventResponse{
				EventType:   tResponseType,
				EventSource: tResponseSource,
			},
...
// Deployed: Unknown
func newEventTargetUnknownDeployed(adapterExists bool) *v1alpha1.HTTPTarget {
	o := newEventTarget()
...
// Deployed: True
func newEventTargetDeployed() *v1alpha1.HTTPTarget {
	o := newEventTarget()
...
// Deployed: False
func newEventTargetNotDeployed() *v1alpha1.HTTPTarget {
	o := newEventTarget()
