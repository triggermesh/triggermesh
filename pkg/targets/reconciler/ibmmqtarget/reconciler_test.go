...
// reconcilerCtor returns a Ctor for a IBMMQTarget Reconciler.
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		r := &Reconciler{
			base:       NewTestServiceReconciler(ctx, ls),
			adapterCfg: cfg,
			trgLister:  ls.GetIBMMQTargetLister().IBMMQTargets,
		}

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetIBMMQTargetLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newTarget returns a populated target object.
func newTarget() *v1alpha1.IBMMQTarget {
	trg := &v1alpha1.IBMMQTarget{
		Spec: v1alpha1.IBMMQTargetSpec{
			ConnectionName: "queue.example.com(1414)",
			QueueManager:   "test-manager",
