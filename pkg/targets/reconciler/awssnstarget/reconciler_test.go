...
// reconcilerCtor returns a Ctor for a AWSSNSTarget Reconciler.
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		...
		r.base = NewTestServiceReconciler(
			ctx,
			ls,
			func(namespace string) common.Lister[*v1alpha1.AWSSNSTarget] {
				return ls.GetAWSSNSTargetLister().AWSSNSTargets(namespace)
			},
		)

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetAWSSNSTargetLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newTarget returns a populated target object.
func newTarget() *v1alpha1.AWSSNSTarget {
	trg := &v1alpha1.AWSSNSTarget{
		Spec: v1alpha1.AWSSNSTargetSpec{
