...
// reconcilerCtor returns a Ctor for a HasuraTarget Reconciler.
var reconcilerCtor Ctor = func(t *testing.T, ctx context.Context, ls *Listers) controller.Reconciler {
	...
	return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
		fakeinjectionclient.Get(ctx), ls.GetHasuraTargetLister(),
		controller.GetEventRecorder(ctx), r)
}

/* Event targets */

// newEventTarget returns a test HasuraTarget object with pre-filled attributes.
func newEventTarget() *v1alpha1.HasuraTarget {
	o := &v1alpha1.HasuraTarget{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: tNs,
			Name:      tName,
			UID:       tUID,
		},
		Spec: v1alpha1.HasuraTargetSpec{
...
// Deployed: Unknown
func newEventTargetUnknownDeployed(adapterExists bool) *v1alpha1.HasuraTarget {
	o := newEventTarget()
...
// Deployed: True
func newEventTargetDeployed() *v1alpha1.HasuraTarget {
	o := newEventTarget()
...
// Deployed: False
func newEventTargetNotDeployed() *v1alpha1.HasuraTarget {
	o := newEventTarget()
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{
				*kmeta.NewControllerRef(NewOwnerRefable(
					tName,
					(&v1alpha1.HasuraTarget{}).GetGroupVersionKind(),
					tUID,
				)),
			},
		},
