...
// reconcilerCtor returns a Ctor for a ConfluentTarget Reconciler.
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		...
		r.base = NewTestServiceReconciler(
			ctx,
			ls,
			func(namespace string) common.Lister[*v1alpha1.ConfluentTarget] {
				return ls.GetConfluentTargetLister().ConfluentTargets(namespace)
			},
		)

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetConfluentTargetLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newTarget returns a populated target object.
func newTarget() *v1alpha1.ConfluentTarget {
	trg := &v1alpha1.ConfluentTarget{
		Spec: v1alpha1.ConfluentTargetSpec{
