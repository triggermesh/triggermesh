...
// reconcilerCtor returns a Ctor for a TwilioTarget Reconciler.
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		...
		r.base = NewTestServiceReconciler(
			ctx,
			ls,
			func(namespace string) common.Lister[*v1alpha1.TwilioTarget] {
				return ls.GetTwilioTargetLister().TwilioTargets(namespace)
			},
		)

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetTwilioTargetLister(),
			controller.GetEventRecorder(ctx), r)
	}
}

// newTarget returns a populated target object.
func newTarget() *v1alpha1.TwilioTarget {
	trg := &v1alpha1.TwilioTarget{
		Spec: v1alpha1.TwilioTargetSpec{},
	}
