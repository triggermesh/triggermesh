...
// reconcilerCtor returns a Ctor for a CloudEventsTarget Reconciler.
func reconcilerCtor(cfg *adapterConfig) Ctor {
	return func(t *testing.T, ctx context.Context, _ *rt.TableRow, ls *Listers) controller.Reconciler {
		...
		r.base = NewTestServiceReconciler[*v1alpha1.CloudEventsTarget](ctx, ls,
			ls.GetCloudEventsTargetLister().CloudEventsTargets,
		)

		return reconcilerv1alpha1.NewReconciler(ctx, logging.FromContext(ctx),
			fakeinjectionclient.Get(ctx), ls.GetCloudEventsTargetLister(),
