...
const (
	...
	tInstrumentName   = "instrument"
	tInstrumentKind   = "Histogram"
	tInstrumentNumber = "Float64"
	tBridge           = "test-bridge"

	tInstrumentEnv = `[{"name":"instrument","description":"instrument description","instrument":"Histogram","number":"Float64"}]`
)

var (
	// We need a pointer to a string for this parameter
	tInstrumentDescription = "instrument description"
...
// newEventTarget returns a test LogzMetricsTarget object with pre-filled attributes.
func newEventTarget() *v1alpha1.LogzMetricsTarget {
	o := &v1alpha1.LogzMetricsTarget{
		...
		Spec: v1alpha1.LogzMetricsTargetSpec{
			Connection: v1alpha1.LogzMetricsConnection{
				ListenerURL: tLogsListenerURL.String(),
			...
			Instruments: []v1alpha1.Instrument{
				{
					...
					Description: &tInstrumentDescription,
					Instrument:  tInstrumentKind,
					Number:      tInstrumentNumber,
				},
			},
		},
	}
...
// Deployed: True
func newEventTargetDeployed() *v1alpha1.LogzMetricsTarget {
	o := newEventTarget()
...
// Deployed: False
func newEventTargetNotDeployed() *v1alpha1.LogzMetricsTarget {
	o := newEventTarget()
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{
				*kmeta.NewControllerRef(NewOwnerRefable(
					tName,
					(&v1alpha1.LogzMetricsTarget{}).GetGroupVersionKind(),
					tUID,
				)),
			},
		},
