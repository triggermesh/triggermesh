...
CUSTOM_BUILD_BINARIES := ibmmqsource-adapter ibmmqtarget-adapter xslttransformation-adapter
...
release: ## Publish container images and generate release manifests
	...
	@if [[ "$(IMAGE_TAG)" =~ ^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)(\-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?(\+[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?$$ ]]; then \
