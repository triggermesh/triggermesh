...
var _ = Describe("Azure Event Grid source", func() {
	...
	Context("a source subscribes to events from a resource group", func() {
		...
		When("an event is generated inside the resource group", func() {
			...
			// NOTE(antoineco): Occasionally, Azure seems to not generate the event related to the above
			...
			// Ref. https://github.com/triggermesh/triggermesh/issues/522
			Specify("the source generates an event", FlakeAttempts(3), func() {
				...
				AddReportEntry("received_events", &receivedEvents, ReportEntryVisibilityFailureOrVerbose)

				readReceivedEvents := readReceivedEvents(f.KubeClient, ns, sink.Ref.Name, &receivedEvents)

				// Although we only create one resource inside our resource group, it is very likely
				// that we receive unrelated events generated by Azure itself while setting up the Event
				// Grid subscription.
				...
				Eventually(readReceivedEvents, receiveTimeout, pollInterval).Should(ContainElement(
					cloudEventDataContains([]byte(`"operationName":"Microsoft.Storage/storageAccounts/write"`)),
					&filteredEvents,
				), "No event with data matching the expected operationName attribute")
...
// cloudEventDataContains returns a Gomega matcher which asserts that a
// CloudEvent's data contains the given bytes.
func cloudEventDataContains(subslice []byte) types.GomegaMatcher {
	return Satisfy(func(e cloudevents.Event) bool {
